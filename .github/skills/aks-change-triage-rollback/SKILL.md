...
3. Remediation (least‑risk first)
   ...
   - For `ImagePullBackOff` due to a missing image (events show `ErrImagePull` with "NotFound"/"manifest unknown"), do not restart first: it only retries the same tag. Verify a prior known-good revision or GitOps last-known-good image, then roll back the scoped deployment.
