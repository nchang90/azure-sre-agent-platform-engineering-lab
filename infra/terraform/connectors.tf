locals {
  ...
  resolved_azure_monitor_id    = var.azure_monitor_resource_id != "" ? var.azure_monitor_resource_id : (local.scenario_value == "s3" ? local.resolved_law_id : "/subscriptions/${data.azurerm_subscription.current.subscription_id}")
