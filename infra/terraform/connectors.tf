locals {
  ...
  resolved_azure_monitor_id    = var.azure_monitor_resource_id != "" ? var.azure_monitor_resource_id : (local.aks_enabled ? azurerm_resource_group.agent.id : "/subscriptions/${data.azurerm_subscription.current.subscription_id}")
