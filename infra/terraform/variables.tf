...
variable "action_mode" {
  description = "Review = human approval. Autonomous = agent acts independently. Automatic is accepted as a legacy alias."
  type        = string
  default     = "Review"

  validation {
    condition     = contains(["Review", "Autonomous", "Automatic"], var.action_mode)
    error_message = "action_mode must be Review, Autonomous, or Automatic."
  }
}
...
variable "webapp_image" {
  ...
  type        = string
  default     = "mcr.microsoft.com/openjdk/jdk:17-ubuntu"
}
...
variable "webapp_port" {
  ...
  validation {
    ...
    error_message = "webapp_port must be between 1 and 65535."
  }
}
