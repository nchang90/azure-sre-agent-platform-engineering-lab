...
- **AKS**:
  [Pod failures](aks-pod-failures.md),
  [network connectivity](aks-network-connectivity.md),
  [resource exhaustion](aks-resource-exhaustion.md)
- **Monitoring**: Log Analytics queries, alert tuning

## 🏗️ Architecture

- [Orders API Architecture](runbooks/containers/orders-architecture.md) -
...
1. **Incident happens** → Find the relevant document in this directory or in
   [runbooks/](runbooks/), including the AKS quick-reference docs:
   [pod failures](aks-pod-failures.md),
   [network connectivity](aks-network-connectivity.md), and
   [resource exhaustion](aks-resource-exhaustion.md)
