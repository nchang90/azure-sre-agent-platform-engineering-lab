...
- [S2 AI Web App Production Incident (Autonomous Remediation)](./s2-autonomous-remediation/)
...
| S2 AI Web App production incident | `environments/sbox.tfvars` | `bash scripts/apply-extras.sh sbox` | Configure GitHub in Azure SRE Agent portal (only if needed) |
| S3 AKS incident remediation and HTTP trigger | `environments/demo.tfvars` (or any tfvars with `scenario = "s3"`) | `bash scripts/apply-extras.sh <environment>` | Set `webhook_bridge_trigger_url` to show the Action Group to HTTP trigger path |
