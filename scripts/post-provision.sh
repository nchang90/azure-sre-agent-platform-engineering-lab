...
register_subagent() {
  ...
  if is_ok_status "$code"; then ok "  Registered: $name"; else warn "  $name returned HTTP $code"; fi
}
...
register_response_plan_file() {
  ...
  if is_ok_status "$code" 409; then ok "  Response plan -> ${handling_agent} (${plan_id})"; else warn "  ${plan_id} returned HTTP $code"; fi
}
...
upload_knowledge_base() {
  ...
  if is_ok_status "$code"; then ok "  Uploaded:$names"; else warn "  Knowledge base upload returned HTTP $code"; fi
  echo
}
...
upload_skills() {
  ...
  for f in .github/skills/*/SKILL.md; do
    ...
    if is_ok_status "$code"; then ok "  Skill: $name"; else warn "  Skill $name returned HTTP $code"; fi
  done
  echo
}
